...
class DGCNN(nn.Module):
    ...
    def sortpooling_embedding(self, node_feat, edge_feat, n2n_sp, e2n_sp, subg_sp, graph_sizes, node_degs):
        ...
        while lv < len(self.latent_dim):
            ...
            if cmd_args.batch_norm:
                batch_norm_linear = self.batch_norms[lv](node_linear)
                normalized_linear = batch_norm_linear.div(node_degs)
            else:
        ...
        if cmd_args.batch_norm:
            conv1d_res = self.batch_norms1(conv1d_res)
        conv1d_res = F.relu(conv1d_res)
        conv1d_res = self.maxpool1d(conv1d_res)
        conv1d_res = self.conv1d_params2(conv1d_res)
        if cmd_args.batch_norm:
            conv1d_res = self.batch_norms2(conv1d_res)
        conv1d_res = F.relu(conv1d_res)
